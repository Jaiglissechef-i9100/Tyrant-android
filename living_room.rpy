...
label lroom22talk:
    ...
    if lroom22first == True:
        ...
        call screen skiplroom22

        screen skiplroom22():
            default tt = Tooltip (" ")

            fixed:
                imagebutton auto "gui/icons/icon_approve_%s.png" xpos 348 ypos 550 action (Hide('skiplroom22'), Jump('skiplroom22yes')) hovered tt.Action ("Yes") focus_mask True
                imagebutton auto "gui/icons/icon_disapprove_%s.png" xpos 348 ypos 750 action (Hide('skiplroom22'), Jump('lroom22con')) hovered tt.Action ("No") focus_mask True

                frame:
                    xalign .5
                    text tt.value

        label skiplroom22yes:
            "Would you collect corruption or love points?"
            call screen lroom22points

            screen lroom22points():
                default tt = Tooltip (" ")

                fixed:
                    imagebutton auto "gui/icons/icon_corruption_%s.png" xpos 348 ypos 550 action (Hide('lroom22points'), Jump('lroom22cpoints')) hovered tt.Action ("Collect [cr1] points") focus_mask True
                    imagebutton auto "gui/icons/icon_love_%s.png" xpos 348 ypos 750 action (Hide('lroom22points'), Jump('lroom22lpoints')) hovered tt.Action ("Collect [lv1] points") focus_mask True

                    frame:
                        xalign .5
                        text tt.value

        label lroom22cpoints:
            $ momcorruption += 1
            $ momrelationship += 1
            $ dtime += 1
            jump lroom

        label lroom22lpoints:
            $ momlove += 1
            $ momrelationship += 1
            $ dtime += 1
            jump lroom
