...
label weekendskip:
    ...
    $ dtime = 7
    $ weekend = False
    $ momtellweekend = False
    $ alexistellweekend = False
    $ cassandratellweekend = False
    $ irinatellweekend = False
    jump mcroom
...
label breakfast:
    ...
    ls "<giggle>"
    pov "{i}From whom should I tell her?{/i}"
    "You can make multiple choices. Just end your selection with \"Finish choosing\"."
    jump momtelllabel

label momtelllabel:
    scene weekend 9am 008
    call screen momtellchoose

screen momtellchoose():
    default tt = Tooltip (" ")

    fixed:
        imagebutton auto "gui/icons/icon_unihand_%s.png" xpos 567 ypos 123 action (Hide ('momtellchoose'), Jump('momtellmom')) hovered tt.Action ("Tell her about herself") focus_mask True
        imagebutton auto "gui/icons/icon_unihand_%s.png" xpos 775 ypos 123 action (Hide ('momtellchoose'), Jump('momtellalexis')) hovered tt.Action ("Tell her about [ls]") focus_mask True
        imagebutton auto "gui/icons/icon_unihand_%s.png" xpos 975 ypos 123 action (Hide ('momtellchoose'), Jump('momtellcassandra')) hovered tt.Action ("Tell her about [bs]") focus_mask True
        imagebutton auto "gui/icons/icon_unihand_%s.png" xpos 1175 ypos 123 action (Hide ('momtellchoose'), Jump('momtellirina')) hovered tt.Action ("Tell her about [irina]") focus_mask True
        imagebutton auto "gui/icons/icon_unihand_%s.png" xpos 1375 ypos 123 action (Hide ('momtellchoose'), Jump('momtellno')) hovered tt.Action ("Don't tell her anything") focus_mask True
        imagebutton auto "gui/icons/icon_abort_%s.png" xpos 975 ypos 323 action (Hide ('momtellchoose'), Jump('momtelldone')) hovered tt.Action ("Finish choosing") focus_mask True
...
label momtellmom:
    $ momtellweekend = True
    "You'll tell her about herself."
    jump momtelllabel

label momtellalexis:
    $ alexistellweekend = True
    "You'll tell her about [ls]."
    jump momtelllabel

label momtellcassandra:
    $ cassandratellweekend = True
    "You'll tell her about [bs]."
    jump momtelllabel

label momtellirina:
    $ irinatellweekend = True
    "You'll tell her about [irina]."
    jump momtelllabel

label momtellno:
    pov "I won't tell you anything."
    mom "Oh you want to have a secret?"
    ls "<giggle>"
    jump weekendcon
...
label momtelldone:
    scene weekend 9am 009
    pov "You're right, I already found some girls and maybe I'll spend some time with them."
    mom "Some girls?"
    ls "Huh?"
    pov "Yes I haven't decided yet."
    if alexistellweekend == True and lilsiscorruption > lilsislove:
        scene weekend 9am 010ac
        pov "One is a cute and beautiful girl. But she's inexperienced, so I need to convince her that she meet my requirements."
        ls "Hnng..."
        if adate == True:
            pov "I already had a date with her."
        if alexisvirgin == False:
            pov "And I took her virginity."
            ls "HNNG..."
        pov "{i}Hah, she's afraid that they'll find out I mean her.{/i}"
    if alexistellweekend == True and lilsiscorruption <= lilsislove:
        scene weekend 9am 010al
        pov "One is a cute and beautiful girl. But we take it slow, because she's inexperienced."
        ls "<giggle>"
        if adate == True:
            pov "I already had a date with her."
        if alexisweekendsecond == True:
            pov "And we had much fun together doing sexual stuff."
            ls "Hmm..."
        pov "{i}Hah, she knows that I talk about her.{/i}"
    if momtellweekend == True and momcorruption > momlove:
        scene weekend 9am 010nc
        pov "The other one is a hot MILF, but she's restive, so I need to train her more!"
        mom "Hnng..."
        if ndate21 == True:
            pov "I already had a date with her."
            if momfirstfuck == True:
                pov "And I fucked her good."
                mom "HNNG..."
        if basement10pmnicoleouting == True:
            pov "And she became my slut, so I'll have much fun with her!"
            mom "..."
        pov "{i}She's afraid that they'll find out, haha.{/i}"
    if momtellweekend == True and momcorruption <= momlove:
        scene weekend 9am 010nl
        pov "The other one is a hot MILF, which is surprising interested in me too, so we have a gentle relationship."
        mom "Oh...?"
        if ndate21 == True:
            pov "I already had a date with her."
            if momdateorgasm == True:
                pov "And I tasted her juices."
                mom "<giggle>"
        if mombasementlovesecond == True:
            pov "And I already made out with her."
        pov "{i}She knows it.{/i}"
    if cassandratellweekend == True and bigsiscorruption > bigsislove:
        scene weekend 9am 010cc
        pov "And one that wants to be a celebrity, but she isn't slutty enough so I'll have to teach her more."
        bs "Eh...?"
        if cdate5 == True:
            pov "I already had a date with her."
            if cdate5fuck == True:
                pov "And we fucked together."
                bs "Hnng..."
            if cdate5bj == True:
                pov "And she sucked my dick."
                bs "Hnng..."
        if basement10cassandraouting == True:
            pov "And she became my slut, so I'll have much fun with her!"
            bs "..."
    if cassandratellweekend == True and bigsiscorruption <= bigsislove:
        scene weekend 9am 010cl
        pov "And there's even another one. She wants to be a celebrity, but I want to protect her and make sure she won't take a slutty way."
        bs "Hmm...?"
        if cdate5 == True:
            pov "I already had a date with her."
            if cdate5hj == True:
                pov "And she played with my dick."
                bs "Hmm..."
        if caslovefirstfuck == True:
            pov "And I had very good sex with her."
            bs "<giggle>"
    if irinatellweekend == True and irinacorruption > irinalove:
        scene weekend 9am 010acnc
        pov "And another one that is already really kinky, so I can enjoy her."
    if irinatellweekend == True and irinacorruption <= irinalove:
        scene weekend 9am 010alnl
        pov "And another one that is lost in that bad neighborhood, but I'll try to rescue her."
    if momtellweekend == True and momcorruption > momlove and alexistellweekend == True and lilsiscorruption > lilsislove:
        scene weekend 9am 010acnc
        mom "Hnng..."
        ls "Hnn..."
        pov "So my answer isn't like the way you expected, haha?"
        "They sit in silence and confused."
    if momtellweekend == True and momcorruption <= momlove and alexistellweekend == True and lilsiscorruption <= lilsislove:
        scene weekend 9am 010alnl
        mom "Hmm..."
        ls "Hmm..."
        pov "So my answer pleased you?"
        "They sit there happy and smiling."
    if momtellweekend == True and momcorruption <= momlove and alexistellweekend == True and lilsiscorruption > lilsislove:
        scene weekend 9am 010acnl
        mom "Hmm..."
        ls "Hnn..."
        pov "{i}Haha, [mother] liked my answer, [ls] is confused.{/i}"
    if momtellweekend == True and momcorruption > momlove and alexistellweekend == True and lilsiscorruption <= lilsislove:
        scene weekend 9am 010alnc
        mom "Hnng..."
        ls "Hmm..."
        pov "{i}Haha, [ls] liked my answer, [mother] is confused.{/i}"
    if momtellweekend == True and alexistellweekend == False or momtellweekend == False and alexistellweekend == True or momtellweekend == False and alexistellweekend == False:
        scene weekend 9am 009
        mom "Hnng..."
        ls "Hnn..."
        pov "So my answer isn't like the way you expected, haha?"
